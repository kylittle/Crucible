...
impl Perlin {
    ...
    pub fn noise(&self, p: &Point3) -> f64 {
        ...
        for (di, i_item) in c.iter_mut().enumerate() {
            for (dj, j_item) in i_item.iter_mut().enumerate() {
                for (dk, point) in j_item.iter_mut().enumerate() {
                    let di_num = di as i32;
                    let dj_num = dj as i32;
                    let dk_num = dk as i32;
                    *point = self.rand_vec[(self.perm_x[((i + di_num) & 255) as usize]
...
fn perlin_interp(c: [[[Vec3; 2]; 2]; 2], u: f64, v: f64, w: f64) -> f64 {
    ...
    for (i, i_item) in c.iter().enumerate() {
        for (j, j_item) in i_item.iter().enumerate() {
            for (k, point) in j_item.iter().enumerate() {
